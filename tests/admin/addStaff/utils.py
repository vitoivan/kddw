from tests.admin.addStaff.mocks import admin, new_user_valid, new_user_login
import json
...
def add_staff(data, client):
	...
	res = client.post(
		url_for('admin_blueprint.add_staff', id=data['id']), 
        headers=headers,
	)	
	client.application.db.session.rollback()
	return res
...
def remove_staff(data, client):
	...
	res = client.post(
		url_for('admin_blueprint.remove_staff', id=data['id']), 
        headers=headers,
	)	
	client.application.db.session.rollback()
	return res
